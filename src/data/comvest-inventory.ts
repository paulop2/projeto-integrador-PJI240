...
/**
 * Inventory of the multiple-choice questions from the Comvest/Unicamp first
 * phase available in the public BLUEX dataset. The module is intentionally
 * read-only and metadata-only: it never carries the question text, the
 * alternatives text or the image bytes, only the references needed to plan and
 * verify an import.
 */

export const comvestFirstPhaseAlternatives = 4;
...
export const comvestAnomalySchema = z.enum([
  'empty-answer',
  'unexpected-alternative-count',
  'image-reference-missing',
]);

export type ComvestAnomaly = z.infer<typeof comvestAnomalySchema>;

export interface ComvestInventoryRecord {
  ...
  readonly anomalies: readonly ComvestAnomaly[];
}

export interface ComvestInventorySourceEntry {
  readonly file: string;
  readonly raw: unknown;
}

export interface ComvestParseError {
  readonly file: string;
  readonly message: string;
}

export interface ComvestDuplicateId {
  readonly id: string;
  readonly files: readonly string[];
}

export interface ComvestMissingImageReference {
  readonly sourceId: string;
  readonly file: string;
  readonly imageRef: string;
}

export interface ComvestYearSummary {
  readonly year: number;
  readonly days: readonly { readonly day: number | null; readonly questions: number }[];
  readonly questions: number;
}

export interface ComvestInventorySummary {
  readonly source: 'bluex';
  readonly university: 'unicamp';
  readonly phase: 'first';
  readonly questionCount: number;
  readonly parseErrorCount: number;
  readonly parseErrors: readonly ComvestParseError[];
  readonly byYear: readonly ComvestYearSummary[];
  ...
  readonly missingImageReferences: readonly ComvestMissingImageReference[];
  readonly orphanImageFiles: readonly string[];
  readonly duplicateIds: readonly ComvestDuplicateId[];
...
export interface SummarizeComvestOptions {
...
export interface ComvestInventoryBuild {
  readonly records: readonly ComvestInventoryRecord[];
  readonly parseErrors: readonly ComvestParseError[];
}

export const buildComvestInventory = (
  entries: readonly ComvestInventorySourceEntry[],
): ComvestInventoryBuild => {
  const records: ComvestInventoryRecord[] = [];
  const parseErrors: ComvestParseError[] = [];

  for (const entry of entries) {
    try {
      records.push(toComvestInventoryRecord(entry.file, entry.raw));
    } catch (error) {
      parseErrors.push({
...
const countByYears = (
  records: readonly ComvestInventoryRecord[],
): readonly ComvestYearSummary[] => {
...
const countAlternativeCounts = (
  records: readonly ComvestInventoryRecord[],
): ComvestInventorySummary['alternativeCountDistribution'] => {
...
export const summarizeComvestInventory = (
...
): ComvestInventorySummary => {
  const { records, parseErrors } = build;
  const referenced = new Set<string>();
  const missingImageReferences: ComvestMissingImageReference[] = [];
  ...
  return {
    ...
    unexpectedAlternativeCounts: records
      .filter(({ alternativeCount }) => alternativeCount !== comvestFirstPhaseAlternatives)
