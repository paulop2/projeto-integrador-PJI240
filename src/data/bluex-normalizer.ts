...
/**
 * Normalizes a BLUEX first-phase multiple-choice snapshot into the runtime
 * `single-choice` contract for any board.
 *
 ...
 * All board differences (identity, edition key, asset prefix) come from
 * {@link BluexBoard}; the parsing logic is shared by Comvest and Fuvest.
 *
...
export const bluexEditionId = (
  board: BluexBoard,
  year: number,
  day: number | null,
): string => (day === null ? `${board.examId}-${year}` : `${board.examId}-${year}-day${day}`);

/** Public URL of an asset published under the app data layout. */
export const bluexAssetUrl = (board: BluexBoard, sourceImagePath: string): string => {
  const prefix = new RegExp(`^imgs/${board.sourceUniversityDir}/`);
  return `/data/${board.examId}/assets/${normalizeSourcePath(sourceImagePath).replace(prefix, '')}`;
};
...
/** Reverses {@link bluexAssetUrl} back to the BLUEX image path. */
export const toBluexSourceImagePath = (board: BluexBoard, assetUrl: string): string =>
  `imgs/${board.sourceUniversityDir}/${normalizeSourcePath(assetUrl).replace(
    `/data/${board.examId}/assets/`,
    '',
  )}`;

export type BluexRejectionReason =
...
export interface BluexRejection {
  readonly sourceFile: string;
  readonly sourceId: string | null;
  readonly editionId: string | null;
  readonly reason: BluexRejectionReason;
  readonly detail: string | null;
}

export type BluexIgnoredAssetReason =
  | 'orphan-file'
  | 'unreferenced-associated-image'
  | 'rejected-question-image';

export interface BluexIgnoredAsset {
  readonly path: string;
  readonly reason: BluexIgnoredAssetReason;
}

export interface BluexEditionPackage {
...
export interface BluexNormalizationResult {
  readonly packages: readonly BluexEditionPackage[];
...
export interface BluexNormalizationOptions {
...
type AlternativeResult =
  ...
  | { readonly ok: false; readonly reason: BluexRejectionReason; readonly detail: string };
...
const parseAlternative = (
  ...
  const file = parsed.imageRefs[0] !== undefined ? bluexAssetUrl(board, parsed.imageRefs[0]) : null;
...
export interface NormalizedBluexQuestion {
...
export type BluexQuestionResult =
  | { readonly ok: true; readonly value: NormalizedBluexQuestion }
  | { readonly ok: false; readonly rejection: BluexRejection };

export const normalizeBluexQuestion = (
  sourceFile: string,
  raw: unknown,
  board: BluexBoard,
): BluexQuestionResult => {
  ...
  let record: BluexInventoryRecord;
  try {
    source = bluexQuestionSchema.parse(raw);
    record = toBluexInventoryRecord(normalizedFile, raw, board);
  } catch (error) {
    return {
      ok: false,
      rejection: {
  ...
  const editionId = bluexEditionId(board, record.year, record.day);
  const reject = (reason: BluexRejectionReason, detail: string): BluexQuestionResult => ({
    ok: false,
    rejection: {
      sourceFile: normalizedFile,
  ...
  for (const rawAlternative of source.alternatives) {
    const parsed = parseAlternative(board, rawAlternative, associatedImages);
  ...
  const question: Question = {
    id: `${board.examId}-${editionId}-${record.sourceId}`,
    ...
    files: [...questionText.imageRefs].map((image) => bluexAssetUrl(board, image)),
...
const sortQuestions = (
  left: NormalizedBluexQuestion,
  right: NormalizedBluexQuestion,
): number =>
...
export const createBluexPackages = (
  entries: readonly BluexInventorySourceEntry[],
  board: BluexBoard,
  options: BluexNormalizationOptions = {},
): BluexNormalizationResult => {
  const rejections: BluexRejection[] = [];
  const accepted: NormalizedBluexQuestion[] = [];
  const seenIds = new Map<string, string>();

  for (const entry of entries) {
    const result = normalizeBluexQuestion(entry.file, entry.raw, board);
    if (!result.ok) {
  ...
  const byEdition = new Map<string, NormalizedBluexQuestion[]>();
  ...
  const packages: BluexEditionPackage[] = [...byEdition.entries()]
  ...
  const ignoredAssets: BluexIgnoredAsset[] = options.availableImageFiles
  ...
  return {
    ...
    referencedAssets: unique(
      accepted.flatMap(({ referencedImages }) =>
        referencedImages.map((image) => bluexAssetUrl(board, image)),
      ),
    ),
    ignoredAssets,
  };
