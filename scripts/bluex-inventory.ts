...
import { buildBluexInventory, summarizeBluexInventory } from '../src/data/bluex-inventory';
import { collectAvailableImages, exists, readSourceEntries, sha256File } from './bluex-source';

export interface BluexInventoryOptions {
  source: string;
  output: string | null;
  pretty: boolean;
...
export const bluexInventoryUsage = (board: BluexBoard): string =>
  `Uso: npm run inventory:${board.id} -- --source <raiz-extraida-do-BLUEX> [opções]

Opções:
  --source DIR                Raiz extraída do BLUEX (com questions/${board.sourceUniversityDir} e imgs/${board.sourceUniversityDir}).
...
export const parseBluexInventoryArgs = (args: readonly string[]): BluexInventoryOptions => {
...
export const runBluexInventory = async (
  board: BluexBoard,
  options: BluexInventoryOptions,
  generatedBy: string,
): Promise<void> => {
  ...
  let zipProvenance: { fileName: string; sha256: string; expected: string | null; verified: boolean } | null =
    null;
  if (options.zipPath) {
  ...
  const build = buildBluexInventory(entries, board);
  const summary = summarizeBluexInventory(build, board, { availableImageFiles });
...
export const runBluexInventoryCli = async (
  board: BluexBoard,
  ...
  try {
    await runBluexInventory(
      board,
      parseBluexInventoryArgs(process.argv.slice(2)),
      `scripts/${scriptFileName}`,
    );
  } catch (error) {
    process.stderr.write(
      `${error instanceof Error ? error.message : String(error)}\n\n${bluexInventoryUsage(board)}`,
    );
    process.exitCode = 1;
  }
};
